...
static void initData(ModifierData *md)
{
  ...
  /* applies to metaball mode, for voxel mode we find those settings on the boolean objects */
  vmd->input = MOD_VOXELMESHER_VERTICES | MOD_VOXELMESHER_PARTICLES;
  vmd->pflag |= eVoxelMesherFlag_Alive;
  vmd->psys = 1;
...
static ParticleSystem *get_psys(
    VoxelMesherModifierData *vmd, CSGVolume_Object *vcob, Object *ob, Scene *scene, bool render)
{
  ParticleSystem *psys;
...
#  if defined WITH_OPENVDB
    ...
    VoxelMesherModifierData *vmd_orig = (VoxelMesherModifierData *)BKE_modifiers_findby_name(
        ob_orig, md->name);
...
static void panel_draw(const bContext *UNUSED(C), Panel *panel)
{
  ...
  if (mode == MOD_VOXELMESHER_VOXEL) {
    uiItemR(layout, ptr, "voxel_size", 0, NULL, ICON_NONE);
    uiItemR(layout, ptr, "isovalue", UI_ITEM_R_SLIDER, NULL, ICON_NONE);
    uiItemR(layout, ptr, "adaptivity", UI_ITEM_R_SLIDER, NULL, ICON_NONE);

    row = uiLayoutRowWithHeading(layout, true, IFACE_("Sharpen"));
    uiLayoutSetPropDecorate(row, false);
    sub = uiLayoutRow(row, true);
    uiItemR(sub, ptr, "sharpen_features", 0, "", ICON_NONE);
    sub = uiLayoutRow(sub, true);
    uiLayoutSetActive(sub, RNA_boolean_get(ptr, "sharpen_features"));
    uiItemR(sub, ptr, "edge_tolerance", UI_ITEM_R_SLIDER, "", ICON_NONE);
    uiItemDecoratorR(row, ptr, "edge_tolerance", 0);

    col = uiLayoutColumn(layout, true);
    uiItemR(col, ptr, "live_remesh", 0, NULL, ICON_NONE);
    uiItemR(col, ptr, "smooth_normals", 0, NULL, ICON_NONE);
    uiItemR(col, ptr, "relax_triangles", 0, NULL, ICON_NONE);
    uiItemR(col, ptr, "reproject_data", 0, NULL, ICON_NONE);
    uiItemR(col, ptr, "accumulate", 0, NULL, ICON_NONE);
    uiItemR(col, ptr, "fix_poles", 0, NULL, ICON_NONE);
  }
  else { /* Metaball mode. */
    int input = RNA_enum_get(ptr, "input");
    uiItemR(layout, ptr, "input", 0, NULL, ICON_NONE);

    if (input & MOD_VOXELMESHER_PARTICLES) {
      uiItemR(layout, ptr, "particle_system_index", 0, NULL, ICON_NONE);
      uiItemR(layout, ptr, "filter", 0, NULL, ICON_NONE);
    }
    uiItemR(layout, ptr, "mball_size", 0, NULL, ICON_NONE);
    uiItemR(layout, ptr, "mball_threshold", 0, NULL, ICON_NONE);
    uiItemR(layout, ptr, "mball_resolution", 0, NULL, ICON_NONE);
    uiItemR(layout, ptr, "mball_render_resolution", 0, NULL, ICON_NONE);
    uiItemR(layout, ptr, "use_smooth_shade", 0, NULL, ICON_NONE);

    modifier_vgroup_ui(
        layout, ptr, &ob_ptr, "size_vertex_group", NULL, IFACE_("Size Vertex Group"));
  }
...
static void object_list_item(struct uiList *UNUSED(ui_list),
                             ...
                             int UNUSED(flt_flag))
{
  ...
  uiItemR(row, itemptr, "enabled", 0, "", ICON_NONE);

  if (index == 0) {
    uiItemL(row, IFACE_("Self Object"), ICON_NONE);
  }
  else {
    // uiLayoutSetEmboss(row, UI_EMBOSS_NONE);
    uiLayout *split = uiLayoutSplit(row, 0.25f, false);
    uiItemL(split, "", ICON_NONE);
    uiItemR(split, itemptr, "object", 0, "", ICON_NONE);
  }
}

static void objects_panel_draw(const bContext *C, Panel *panel)
{
  uiLayout *col, *row, *sub;
  uiLayout *layout = panel->layout;
  ...
  int mode = RNA_enum_get(ptr, "mode");
  int active_index = RNA_int_get(ptr, "active_index");

  uiLayoutSetActive(layout, mode == MOD_VOXELMESHER_VOXEL);

  row = uiLayoutRow(layout, false);
  uiLayoutSetPropSep(row, false);
  uiTemplateList(row,
                 (bContext *)C,
                 "MOD_UL_voxelmesher_objects",
                 "",
                 ptr,
                 "csg_operands",
                 ptr,
  ...
  uiItemO(sub, "", ICON_ADD, "VOXELMESHER_OT_csg_add");
  uiItemIntO(sub, "", ICON_REMOVE, "VOXELMESHER_OT_csg_remove", "index", active_index);
  uiItemS(col);
  sub = uiLayoutColumn(col, true);
  uiItemIntO(sub, "", ICON_TRIA_UP, "VOXELMESHER_OT_csg_move_up", "index", active_index);
  uiItemIntO(sub, "", ICON_TRIA_DOWN, "VOXELMESHER_OT_csg_move_down", "index", active_index);

  VoxelMesherModifierData *vmd = ptr->data;
  CSGVolume_Object *csg_object = BLI_findlink(&vmd->csg_operands, active_index);
  BLI_assert(csg_object != NULL);

  PointerRNA csg_ptr;
  RNA_pointer_create(ptr->owner_id, &RNA_CSGVolume_Object, csg_object, &csg_ptr);

  bool use_voxel_percentage = RNA_boolean_get(&csg_ptr, "use_voxel_percentage");
  int input = RNA_enum_get(&csg_ptr, "input");

  uiLayoutSetPropSep(layout, true);

  uiItemR(layout, &csg_ptr, "input", 0, NULL, ICON_NONE);
  uiItemR(layout, &csg_ptr, "operation", 0, NULL, ICON_NONE);
  uiItemR(layout, &csg_ptr, "use_voxel_percentage", 0, NULL, ICON_NONE);

  if (use_voxel_percentage) {
    uiItemR(layout, &csg_ptr, "voxel_percentage", UI_ITEM_R_SLIDER, NULL, ICON_NONE);
  }
  else {
    col = uiLayoutColumn(layout, false);
    uiItemR(col, &csg_ptr, "sync_voxel_size", 0, NULL, ICON_NONE);
    uiItemR(col, &csg_ptr, "voxel_size", 0, NULL, ICON_NONE);
  }

  if (input & MOD_VOXELMESHER_PARTICLES) {
    col = uiLayoutColumn(layout, false);
    uiItemR(col, &csg_ptr, "particle_system_index", 0, NULL, ICON_NONE);
    uiItemR(col, &csg_ptr, "part_min_radius", 0, NULL, ICON_NONE);
    uiItemR(col, &csg_ptr, "part_scale_factor", 0, NULL, ICON_NONE);
    uiItemR(col, &csg_ptr, "part_vel_factor", 0, NULL, ICON_NONE);

    col = uiLayoutColumn(layout, false);
    uiItemR(col, &csg_ptr, "part_trail", 0, NULL, ICON_NONE);
    uiItemR(col, &csg_ptr, "part_trail_size", 0, NULL, ICON_NONE);
  }
}

static void filter_panel_draw(const bContext *UNUSED(C), Panel *panel)
{
  uiLayout *col;
  uiLayout *layout = panel->layout;
  ...
  int mode = RNA_enum_get(ptr, "mode");
  int filter_type = RNA_enum_get(ptr, "filter_type");
  ...
  col = uiLayoutColumn(layout, false);
  uiItemR(col, ptr, "filter_type", 0, NULL, ICON_NONE);
  if (filter_type != VOXEL_FILTER_NONE) {
    uiItemR(col, ptr, "filter_bias", 0, NULL, ICON_NONE);
    if (ELEM(filter_type, VOXEL_FILTER_GAUSSIAN, VOXEL_FILTER_MEDIAN, VOXEL_FILTER_MEAN)) {
      uiItemR(col, ptr, "filter_width", 0, NULL, ICON_NONE);
    }
    if (ELEM(filter_type, VOXEL_FILTER_GAUSSIAN)) {
      uiItemR(col, ptr, "filter_iterations", 0, NULL, ICON_NONE);
      uiItemR(col, ptr, "filter_sigma", 0, NULL, ICON_NONE);
    }
    if (ELEM(filter_type, VOXEL_FILTER_DILATE, VOXEL_FILTER_ERODE)) {
      uiItemR(col, ptr, "filter_distance", 0, NULL, ICON_NONE);
    }
  }
}
...
static void panelRegister(ARegionType *region_type)
{
  PanelType *panel_type = modifier_panel_register(
      region_type, eModifierType_VoxelMesher, panel_draw);
#ifdef WITH_MOD_REMESH
  modifier_subpanel_register(
      region_type, "objects", "Boolean Objects", NULL, objects_panel_draw, panel_type);
  modifier_subpanel_register(region_type, "filter", "Filter", NULL, filter_panel_draw, panel_type);

  uiListType *list_type = MEM_callocN(sizeof(uiListType), "voxelmesher objects uilist");
  strcpy(list_type->idname, "MOD_UL_voxelmesher_objects");
  list_type->draw_item = object_list_item;
...
static void blendWrite(BlendWriter *writer, const ModifierData *md)
{
  ...
  CSGVolume_Object *vcob;
  for (vcob = vmd->csg_operands.first; vcob; vcob = vcob->next) {
    BLO_write_struct(writer, CSGVolume_Object, vcob);
  }
}

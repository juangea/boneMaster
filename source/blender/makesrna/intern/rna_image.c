...
static void rna_ImageTile_label_get(PointerRNA *ptr, char *value)
{
  ImageTile *tile = (ImageTile *)ptr->data;
  Image *image = (Image *)ptr->owner_id;

  /* We don't know the length of the target string here, so we assume
   * that it has been allocated according to what rna_ImageTile_label_length returned. */
  BKE_image_get_tile_label(image, tile, value, 64);
}

static int rna_ImageTile_label_length(PointerRNA *ptr)
{
  ImageTile *tile = (ImageTile *)ptr->data;
  Image *image = (Image *)ptr->owner_id;
...
static int rna_ImageTile_tile_number_get(PointerRNA *ptr)
{
  ImageTile *tile = (ImageTile *)ptr->data;

  return tile->tile_number + 1001;
}

static void rna_ImageTile_tile_number_set(PointerRNA *ptr, int value)
{
  ImageTile *tile = (ImageTile *)ptr->data;
  Image *image = (Image *)ptr->owner_id;
  ...
  int tile_number = value - 1001;

  /* Check that no other tile already has that number. */
  ImageTile *cur_tile = BKE_image_get_tile(image, tile_number);
...
static PointerRNA rna_Image_active_tile_get(PointerRNA *ptr)
{
  ...
  return rna_pointer_inherit_refine(ptr, &RNA_ImageTile, tile);
}
...
static void rna_ImageTile_remove(Image *image, PointerRNA *ptr)
{
  ImageTile *tile = (ImageTile *)ptr->data;
...
static void rna_def_image_tile(BlenderRNA *brna)
{
  ...
  RNA_def_property_string_funcs(
      prop, "rna_ImageTile_label_get", "rna_ImageTile_label_length", NULL);
  RNA_def_property_update(prop, NC_IMAGE | ND_DISPLAY, NULL);
  ...
  RNA_def_property_int_funcs(
      prop, "rna_ImageTile_tile_number_get", "rna_ImageTile_tile_number_set", NULL);
  RNA_def_property_update(prop, NC_IMAGE | ND_DISPLAY, NULL);
}
...
static void rna_def_image_tiles(BlenderRNA *brna, PropertyRNA *cprop)
{
  ...
  RNA_def_struct_sdna(srna, "Image");
  RNA_def_struct_ui_text(srna, "Image Tiles", "Collection of image tiles");
  ...
  parm = RNA_def_pointer(func, "tile", "ImageTile", "", "Image tile to remove");

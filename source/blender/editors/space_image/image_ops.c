...
static int tile_add_exec(bContext *C, wmOperator *op)
{
  ...
  /* Convert between user-facing UDIM number and internal tile number. */
  int tile_number = RNA_int_get(op->ptr, "tile_number") - 1001;
...
static int tile_add_invoke(bContext *C, wmOperator *op, const wmEvent *UNUSED(event))
{
  ...
  LISTBASE_FOREACH (ImageTile *, tile, &ima->tiles) {
    ...
    if (!tile->next || tile->next->tile_number > next_number) {
      break;
    }
  }

  RNA_int_set(op->ptr, "tile_number", 1001 + next_number);
...
static void tile_add_draw(bContext *UNUSED(C), wmOperator *op)
{
  ...
  uiItemL(col[0], IFACE_("Tile Number"), ICON_NONE);
  uiItemR(col[1], &ptr, "tile_number", 0, "", ICON_NONE);
...
void IMAGE_OT_tile_add(wmOperatorType *ot)
{
  ...
  RNA_def_int(
      ot->srna, "tile_number", 1002, 1001, INT_MAX, "Tile Number", "Image height", 1001, 1099);
...
static bool tile_remove_poll(bContext *C)
{
  ...
  return (ima && ima->source == IMA_SRC_TILED && ima->active_tile_index != 0);
}

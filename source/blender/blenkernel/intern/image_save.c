    ...
    ReportList *reports, Main *bmain, Image *ima, ImageUser *iuser, ImageSaveOptions *opts)
{
  ...
  if (ok && ima->source == IMA_SRC_TILED) {
    ...
    LISTBASE_FOREACH (ImageTile *, tile, &ima->tiles) {
      /* Tile 0 was already saved before the loop. */
      if (tile->tile_number == 0 || !ok)
        continue;

      /* Build filepath of the tile. */
      BLI_stringenc(opts->filepath, head, tail, numlen, 1001 + tile->tile_number);

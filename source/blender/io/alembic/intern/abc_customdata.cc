                                   ...
                                   Callback callback)
{
  ...
  // std::cerr << "Adding " << name << " on " << scope_string(bl_scope) << '\n';
                                                 ...
                                                 ISampleSelector iss)
{
  ...
  switch (abc_mapping) {
    ...
    case AbcAttributeMapping::TO_UVS: {
      /* TODO(kevindietrich) : check the number of UV layers. */
    ...
    case AbcAttributeMapping::TO_VERTEX_COLORS_FROM_RGB: {
      /* TODO(kevindietrich) : check the number of mcol layers. */
                               ...
                               float velocity_scale)
{
  ...
  for (ParsedAttributeDesc desc : attributes) {
    ...
    if (IFloatGeomParam::matches(prop)) {
      ...
      else if (prop.getName() == attr_sel.velocity_name() && velocity_scale != 0.0f) {

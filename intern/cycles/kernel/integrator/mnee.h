...
ccl_device_inline float mat22_inverse(const float4 &m, float4 &m_inverse)
{
...
ccl_device_forceinline void mnee_make_orthonormals(const float3 n, float3 *dp_du, float3 *dp_dv)
{
  if (n.z < 0.0f) {
                                                       ...
                                                       ShaderData *sd_vtx)
{
  ...
  /* dp_du and dp_dv need to be continuous across triangles
   * for the h normal offset to yield a consistent halfvector
   * while walking on the manifold. It's usually best to
   * rely on the mesh uv layout, which is assumed to be
  ...
  /* intialize constraint and its derivates */
...
ccl_device_forceinline bool mnee_compute_constraint_derivatives(int vertex_count,
                                                                ManifoldVertex *vertices,
                                                                const float3 &surface_sample_pos,
                                                                const bool light_fixed_direction,
                                                                const float3 &light_sample)
{
  for (int vi = 0; vi < vertex_count; vi++) {

    ManifoldVertex &v = vertices[vi];
    ...
    if (vi > 0) {
      ManifoldVertex &v_prev = vertices[vi - 1];
    ...
    if (vi < vertex_count - 1) {
      ManifoldVertex &v_next = vertices[vi + 1];
  ...
  for (int iteration = 0; iteration < MNEE_MAX_ITERATIONS; iteration++) {
    ...
    for (int vi = 0; vi < vertex_count; vi++) {
      ...
      if (vi == 0) {
        ...
        const ManifoldVertex &pv = vertices[vi - 1];
    ...
    if (!reduce_stepsize) {
      for (int vi = 0; vi < vertex_count; vi++) {
        ManifoldVertex &tv = tentative[vi];
...
ccl_device_forceinline float3 mnee_eval_bsdf_contribution(ShaderClosure *closure,
                                                          float3 wi,
                                                          float3 wo)
{
  MicrofacetBsdf *bsdf = (MicrofacetBsdf *)closure;
...
ccl_device_forceinline bool mnee_compute_transfer_matrix(const ShaderData *sd,
                                                         const LightSample *ls,
                                                         int vertex_count,
                                                         ManifoldVertex *vertices,
                                                         float *dx1_dxlight,
                                                         float *dh_dx)
{
  ...
  const ManifoldVertex &m = vertices[mi];
                                                   ...
                                                   BsdfEval *throughput)
{
  ...
  /* update light sample with new position / direction
  ...
  float wi_len;
  for (int vi = 0; vi < vertex_count; vi++) {
    const ManifoldVertex &v = vertices[vi];
                                                    ...
                                                    BsdfEval *throughput)
{
  ...
  for (int isect_count = 0; isect_count < MNEE_MAX_INTERSECTION_COUNT; isect_count++) {
    ...
    if (object_flags & SD_OBJECT_CAUSTICS_CASTER) {
      ...
      ManifoldVertex &mv = vertices[vertex_count++];
      ...
      for (int ci = 0; ci < sd_mnee->num_closure; ci++) {
        ShaderClosure *bsdf = &sd_mnee->closure[ci];
        if (bsdf->type == CLOSURE_BSDF_MICROFACET_BECKMANN_REFRACTION_ID ||
            bsdf->type == CLOSURE_BSDF_MICROFACET_GGX_REFRACTION_ID) {

          found_transimissive_microfacet_bsdf = true;
          MicrofacetBsdf *microfacet_bsdf = (MicrofacetBsdf *)bsdf;

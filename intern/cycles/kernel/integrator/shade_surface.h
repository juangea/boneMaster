                                                           ...
                                                           ccl_private const RNGState *rng_state)
{
  ...
  {
    if (ls.lamp != LAMP_NONE || ls.type == LIGHT_BACKGROUND) {
      ...
      if (use_caustics) {
        ...
        if (!is_transmission && (sd->object_flag & SD_OBJECT_CAUSTICS_RECEIVER))
          skip_nee = kernel_path_mnee_sample(kg, state, sd, emission_sd, rng_state, &ls, &bsdf_eval);
      }
    }
  }
  if (skip_nee) {
...
#  endif
  ...
  INTEGRATOR_STATE_WRITE(shadow_state, shadow_path, lightgroup) = (ls.type != LIGHT_BACKGROUND) ? ls.group + 1 : kernel_data.background.lightgroup + 1;
}
#endif
